...
def list_files_with_sizes(directory):
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            if not os.path.isfile(filepath) or os.path.isdir(filepath) or os.path.islink(filepath):
                continue
            try:
                filesize = os.path.getsize(filepath)
                files_with_sizes.append((filepath, filesize))
            except OSError as e:
                logging.warning(f"Error getting size of file {filepath}: {e}", file=sys.stderr)
                continue
...
def sample_data(files_with_sizes):
    ...
    if args.verbose:
        ...
        print(f"Total size of files: {total_size} bytes ({human_readable_size(total_size)})")
    ...
    for filepath, filesize in files_with_sizes:
        ...
        try:
            ...
            logging.warning(f"Error: Insufficient permissions to read the file '{filepath}'.")
            continue
        except IsADirectoryError:
            logging.warning(f"Error: '{filepath}' is a directory, not a file.")
            continue
        except TypeError:
            logging.warning("Error: Invalid file path type. Expected a string, bytes, or os.PathLike object.")
            continue
        except OSError as e:
...
def compress_sampled_data(sampled_data):
    ...
    if args.verbose:
        print(f"Compressed sample size: {len(compressed_data)} bytes ({human_readable_size(len(compressed_data))})")
        print(f"Original sample size: {len(sampled_data)} bytes ({human_readable_size(len(sampled_data))})")
...
def main(directory):
    ...
    compressed_size = (float(compressed_size) / len(sampled_data)) * original_size if original_size > 0 else 0

    # Step 5: Print original and estimated compressed size 
    print(f"---------------------------------------------------------------------------------")
    print(f"Original size: {original_size} bytes ({human_readable_size(original_size)})")
    print(f"Estimated compressed size: {compressed_size:.2f} ({human_readable_size(compressed_size)}) ")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Estimate compression size of sampled data.")
    parser.add_argument("directory", nargs="?", help="Directory to process.")
    parser.add_argument("--algorithm", choices=["gzip", "bzip2"], default="gzip", help="Compression algorithm to use (gzip or bzip2).")
    parser.add_argument("--level", type=int, choices=range(1, 10), default=9, help="Compression level (1-9).")
    parser.add_argument("--sampling-ratio", type=float, default=0.1, help="Sampling ratio (fraction of data to sample, e.g., 0.1 for 10%%).")
